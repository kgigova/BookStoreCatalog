...
<asp:Content ID="Content4" ContentPlaceHolderID="MainContent" runat="server">
    <asp:FormView ID="FormView1" runat="server" DataSourceID="SqlDataSource1" DataKeyNames="book_id" >
        <ItemTemplate>
			...
			<div class="article-info"><asp:Label ID="Label2" runat="server" Text='<%# Eval("author_name") %>' /></div>

			 <asp:Image ID="imgBookPic" ImageUrl='<%# Bind("imagePath") %>' runat="server" style="width:30%; height:50%; margin:auto"/>
			
            
            <p><asp:Label ID="Label3" runat="server" Text='<%# Eval("description") %>' /></p>
    ...
    <asp:GridView ID="GridView1" runat="server" DataSourceID="SqlDataSource2" AllowPaging="True" ShowHeader="false" AutoGenerateColumns="false">
        <Columns>
            <asp:TemplateField ShowHeader="false">
                <ItemTemplate>
                    <article class="expanded">
            		<h4><asp:Label ID="Label1" runat="server" Text='<%# Eval("fname") %>' />&nbsp;<asp:Label ID="Label4" runat="server" Text='<%# Eval("lname") %>' /></h4>
			        <div class="article-info"><asp:Label ID="Label2" runat="server" Text='<%# Eval("c_date_time") %>' /></div>
                    <p>
